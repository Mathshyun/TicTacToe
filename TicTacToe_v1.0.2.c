...
#define AI 1
#define TEST 2
#define AIINTERVAL_DEFAULT 1000
#define VER "v1.0.2"
...
void startGame(int type, struct settings set);												//게임을 시작하는 함수
...
struct settings settings(struct settings set);												//설정 화면을 표시하는 함수
struct point TTTAI(int map[3][3], int turn);												//AI의 선택을 결정하는 함수

int main() {
	system("chcp 65001");						//유니코드 설정
	system("mode con:cols=100 lines=30");		//창 크기 조절
	system("title TIC-TAC-TOE by Mathshyun");	//창 제목 설정
	srand(time(NULL));							//랜덤 변수의 시드(seed) 설정
	
	struct settings set;						//설정 구조체
	set.AIinterval = AIINTERVAL_DEFAULT;
...
int title() {
	...
	while (1) {
		...
		printf("                                                                                                    \n");
		printf("                                                                                                    \n");
		printf(" by Mathshyun!                                                                               "); printf(VER);
		switch (x = getch()) {
		case 224:
			x = getch();
			printf("");
...
//게임을 시작하는 함수
void startGame(int type, struct settings set) {
	...
	while (1) {
		if (kbhit()) {															//키보드 입력이 있는지 검사
			...
			if (p[turn - 1] == AI) {
				if ((clock() - prevTime) >= set.AIinterval) {
					int x;
...
void endGame(int map[3][3], int p[2], int wp, int type, int turn, struct point pointer) {
	...
	printf("│ │   %s%s%s%s%s  │   %s%s%s%s%s  │   %s%s%s%s%s  │ │  %s %s %s                            │ │ \n", (map[0][0] == 1) ? "■" : "  ", (map[0][0] == 2) ? "■" : "  ", (map[0][0] == 2) ? "■" : "  ", (map[0][0] == 2) ? "■" : "  ", (map[0][0] == 1) ? "■" : "  ", (map[1][0] == 1) ? "■" : "  ", (map[1][0] == 2) ? "■" : "  ", (map[1][0] == 2) ? "■" : "  ", (map[1][0] == 2) ? "■" : "  ", (map[1][0] == 1) ? "■" : "  ", (map[2][0] == 1) ? "■" : "  ", (map[2][0] == 2) ? "■" : "  ", (map[2][0] == 2) ? "■" : "  ", (map[2][0] == 2) ? "■" : "  ", (map[2][0] == 1) ? "■" : "  ", (turn == 0) ? "▶" : "  ", (p[0] == PLAYER) ? "플레이어" : "   AI   ", (type == 1) ? " " : "X");
	printf("│ │   %s%s  %s%s  │   %s%s  %s%s  │   %s%s  %s%s  │ │  %s %s %s                            │ │ \n", (map[0][0] == 2) ? "■" : "  ", (map[0][0] == 1) ? "■" : "  ", (map[0][0] == 1) ? "■" : "  ", (map[0][0] == 2) ? "■" : "  ", (map[1][0] == 2) ? "■" : "  ", (map[1][0] == 1) ? "■" : "  ", (map[1][0] == 1) ? "■" : "  ", (map[1][0] == 2) ? "■" : "  ", (map[2][0] == 2) ? "■" : "  ", (map[2][0] == 1) ? "■" : "  ", (map[2][0] == 1) ? "■" : "  ", (map[2][0] == 2) ? "■" : "  ", (turn == 1) ? "▶" : "  ", (p[1] == PLAYER) ? "플레이어" : "   AI   ", (type == 1) ? " " : "O");
	...
	printf("│ │   %s%s  %s%s  │   %s%s  %s%s  │   │    %s %c 승!   │                                     │ \n", (map[0][1] == 2) ? "■" : "  ", (map[0][1] == 1) ? "■" : "  ", (map[0][1] == 1) ? "■" : "  ", (map[0][1] == 2) ? "■" : "  ", (map[1][1] == 2) ? "■" : "  ", (map[1][1] == 1) ? "■" : "  ", (map[1][1] == 1) ? "■" : "  ", (map[1][1] == 2) ? "■" : "  ", (p[wp - 1] == PLAYER) ? "플레이어" : "   AI   ", (wp == 1) ? 'X': 'O');
	else
...
//설정 화면을 표시하는 함수
struct settings settings(struct settings set) {
	int x, selected = 0;
	...
	while (1) {
		...
		printf("                                                                                                    \n");
		printf("                                                                                                    \n");
		printf("                                     %s AI 대기 시간 : %s %s                                     \n", (selected == 0) ? "▶" : "  ", (set.AIinterval == 1000) ? " 1초" : (set.AIinterval == 500) ? ".5초" : "즉시", (selected == 0) ? "◀" : "  ");
		printf("                                            %s 나가기 %s                                            \n", (selected == 1) ? "▶" : "  ", (selected == 1) ? "◀" : "  ");
		printf("                                                                                                    \n");
		printf("                                                                                                    \n");
		printf("                                                                                                    \n");
		...
		case 224:
			...
			printf("");
			if (x == 72 && selected != 0) selected--;
			else if (x == 80 && selected != 1) selected++;
			break;
		case 13:
			switch (selected) {
			case 0:
				if (set.AIinterval == 0) set.AIinterval = 1000;
				else set.AIinterval -= 500;
				break;
			case 1:
				return set;
...
struct point TTTAI(int map[3][3], int turn) {
	...
	//무작위의 빈 코너를 선택한다
	int cornerArray[4] = {0, 2, 20, 22};
    for (int i = 0; i < 4; i++) {
        int swapOrder = rand() % 4;
        int swapTemp = cornerArray[swapOrder];
        cornerArray[swapOrder] = cornerArray[i];
        cornerArray[i] = swapTemp;
    }
    for (int i = 0; i < 4; i++) {
        int checkX = cornerArray[i] / 10, checkY = cornerArray[i] % 10;
	...
	//무작위의 빈 사이드를 선택한다
	int sideArray[4] = {1, 10, 12, 21};
    for (int i = 0; i < 4; i++) {
        int swapOrder = rand() % 4;
        int swapTemp = sideArray[swapOrder];
        sideArray[swapOrder] = sideArray[i];
        sideArray[i] = swapTemp;
    }
    for (int i = 0; i < 4; i++) {
        int checkX = sideArray[i] / 10, checkY = sideArray[i] % 10;
        if (map[checkX][checkY] == 0) {
            res.x = checkX;
            res.y = checkY;
            return res;
